...
with open("PeriodicApproxAPFunction.dat","w+") as dataf:
        ...
        for i in range(0, 60):
                ...
                mesh = RectangleMesh(Point(RS,RS), Point(LS, LS), 100+(i+1)*(i+1), 100+(i+1)*(i+1))
                ...
                a = g1*inner(grad(u), grad(v))*dx + c*v*dx + u*d*dx
                L = -g1*grad(v)[0]*dx
